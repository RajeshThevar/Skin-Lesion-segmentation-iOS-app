...
- (void)viewDidLoad {
    ...
    self.grabcut = [[GrabCutManager alloc] init];
    self.activeContour = [[GrabCutManager alloc] init];
  
    _originalImage = [UIImage imageNamed:@"4023d_1.jpg"];
...
// Active Contour Method calling the function present in grabcutManager.mm
#if 0
-(void) doActiveContour {
...
//                                           (unsigned long)NULL), ^(void) {
  ...
  GrabCutManager * activeContour = self.activeContour;
  assert(nil != activeContour);
  
  UIImage* resultImage= [activeContour doActiveContour:self.originalImage];
...
// Here I have typed in IBAction for Active Contour
-(IBAction)tapOnActiveContour:(id)sender{
  [self doActiveContour];
